...
const Conversations = ({navigation, route}: ScreenProps) => {
  ...
  useEffect(() => {
    if (user && selectedConversation) {
      ...
      setMessagesList(selectedConversation.messages);
    }
  ...
  const initPusher = async () => {
    if (selectedConversation) {
      ...
      const channel = await pusher.getChannel(selectedConversation._id);
      if (!channel) {
        await pusher.subscribe({
          ...
          onEvent: (event: PusherEvent) => {
            ...
            dispatch(getConversationByIdApi(selectedConversation._id, token));
          },
        });
      } else {
